...
src = np.float32([[20, 200], [350, 200], [275, 120], [85, 120]]) # координаты точек отображаемой фигуры (трапеция)
...
while cv2.waitKey(1) != 27:
    ...
    # пороги должны быть подобраны заранее

    r_channel = resized[:, :, 2] # извлечение красного канала цвета
    binary = np.zeros_like(r_channel) # массив из 0 размерности как у r_channel
    ...
    cv2.imshow('polylines', allBinary_visual)

    M = cv2.getPerspectiveTransform(src, dst)
    warped = cv2.warpPerspective(allBinary, M, (img_size[1], img_size[0]), flags=cv2.INTER_LINEAR)
    cv2.imshow("warped", warped)

    # Нахождение белых столбцов в нижней части изображения
    histogram = np.sum(warped[warped.shape[0] // 2:, :], axis=0)

    midpoint = histogram.shape[0] // 2 # получение номера серединного столбца
    IndWhiteColumnL = np.argmax(histogram[:midpoint])
    IndWhiteColumnR = np.argmax(histogram[midpoint:]) + midpoint

    warped_visual = warped.copy()
    cv2.line(warped_visual, (IndWhiteColumnL, 0), (IndWhiteColumnL, warped_visual.shape[0]), 110, 2)
    cv2.line(warped_visual, (IndWhiteColumnR, 0), (IndWhiteColumnR, warped_visual.shape[0]), 110, 2)
    ...
    for window in range(nwindows):
        ...
        win_y2 = warped.shape[1] - window * window_height
        ...
        if len(good_left_inds) > 50:
            XCenterLeftWindow = np.int_(np.mean(WhitePixelIndX[good_left_inds]))
        if len(good_right_inds) > 50:
            XCenterRightWindow = np.int_(np.mean(WhitePixelIndX[good_right_inds]))
    ...
    cv2.imshow("colored lanes", out_img)
